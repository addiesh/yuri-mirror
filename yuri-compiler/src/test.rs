...
//     let (ast, errors, _state) = yuri_parser::parse_all(source, &mut storage, &tokens);
//     if !errors.is_empty() {
...
";
    ...
    if !state.errors.is_empty() {
        panic!("Parser returned errors: {:?}", state.errors);
    }

    let name = storage.to_ident("test_module");

    let lowered = crate::lower::lower(source, &mut storage, &ast, name).unwrap();
    let ll = lowered.lock().unwrap();
    println!("{ll:?}");
}
