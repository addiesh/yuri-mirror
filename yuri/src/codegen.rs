...
fn codegen() {
    ...
    let f_1 = b.constant_bit32(f, f32::to_bits(1.0f32));
    ...
    fs::write("yuri.frag.spvasm", disassembled).unwrap();

    let disassembled_bytes: Vec<u8> = assembled.iter().flat_map(|w| w.to_le_bytes()).collect();

    fs::write("yuri.frag.spv", disassembled_bytes).unwrap();
}
